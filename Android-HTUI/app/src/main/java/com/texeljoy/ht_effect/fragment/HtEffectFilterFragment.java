...
import com.texeljoy.ht_effect.adapter.HtEffectFilterItemViewBinder;
 ...
 * 特效滤镜
 */
public class HtEffectFilterFragment extends HtBaseLazyFragment {

  private View container;

  private RecyclerView rvFilter;

  private final List<HtEffectFilter> items = new ArrayList<>();
  ...
  @Override protected void initView(View view, Bundle savedInstanceState) {
    ...
    HtEffectFilterConfig filterList = HtConfigTools.getInstance().getEffectFilterConfig();

    if (filterList == null) {
      HtConfigTools.getInstance().getEffectFiltersConfig(new HtConfigCallBack<List<HtEffectFilter>>() {
        @Override public void success(List<HtEffectFilter> list) {
          items.addAll(list);
          initRecyclerView();
        }
