 ...
 * 贴纸Item的适配器
 */
public class HtGiftAdapter extends RecyclerView.Adapter<HtStickerViewHolder> {

    private final int ITEM_TYPE_ONE = 1;

    private final int ITEM_TYPE_TWO = 2;

    private int selectedPosition = HtSelectedPosition.POSITION_GIFT;
    private int lastPosition;

    private final List<HtGiftConfig.HtGift> giftList;

    private Handler handler = new Handler();

    private Map<String, String> downloadingGifts = new ConcurrentHashMap<>();

    public HtGiftAdapter(List<HtGiftConfig.HtGift> giftList) {
    ...
    public void onBindViewHolder(@NonNull final HtStickerViewHolder holder, int position) {
        ...
        final HtGiftConfig.HtGift htGift = giftList.get(holder.getAdapterPosition());
        selectedPosition = HtSelectedPosition.POSITION_GIFT;
        ...
        //显示封面
        if (htGift == HtGiftConfig.HtGift.NO_Gift) {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //如果没有下载，则开始下载到本地
                if (htGift.isDownloaded() == HTDownloadState.NOT_DOWNLOAD) {
                    ...
                    new DownloadTask.Builder(htGift.getUrl(), new File(HTEffect.shareInstance().getARItemPathBy(HTItemEnum.HTItemGift.getValue())))
                            ...
                            .enqueue(new DownloadListener2() {
                                ...
                                public void taskStart(@NonNull DownloadTask task) {
                                    downloadingGifts.put(htGift.getName(), htGift.getUrl());
                                ...
                                public void taskEnd(@NonNull final DownloadTask task, @NonNull EndCause cause, @Nullable final Exception realCause) {
                                    downloadingGifts.remove(htGift.getName());
                                    ...
                                    if (cause == EndCause.COMPLETED) {
                                        new Thread(new Runnable() {
                                            ...
                                            public void run() {
                                                File targetDir =
                                                    new File(HTEffect.shareInstance().getARItemPathBy(HTItemEnum.HTItemGift.getValue()));
                                                ...
                                                try {
                                                    ...
                                                    //修改内存与文件
                                                    htGift.setDownloaded(HTDownloadState.COMPLETE_DOWNLOAD);
                                                    htGift.downloaded();

                                                    // HTEffect.shareInstance().setARItem(HTItemEnum.HTItemGift.getValue(), htGift.getName());
                                                    BeautyManager.setARItemProperty(HTItemEnum.HTItemGift.getValue(), htGift.getName());
                                                    lastPosition = selectedPosition;
                                                    selectedPosition = currentPosition;
                                                    HtSelectedPosition.POSITION_GIFT = selectedPosition;
