...
import io.agora.rte.extension.hteffect.example.BeautyManager.PathCallback;
...
public class HtConfigTools {
  ...
  public void initHtConfigTools(Context context) {
    ...
    BeautyManager.getPath("htGetAISegEffectPath", new BeautyManager.PathCallback() {
      @Override
      public void onPathReceived(String path) {
        if (path == null) {
          Log.d("returnAISegEffectPath", "AISegEffect path is still null");
        } else {
          Log.d("returnAISegEffectPath", "AISegEffect path: " + path);
          PATH_AISEGMENTATION = path + File.separator + "ht_aiseg_effect_config.json";
        }
      }
    });
    ...
    BeautyManager.getPath("htGetChromaKeyingPath", new BeautyManager.PathCallback() {
      @Override
      public void onPathReceived(String path) {
        if (path == null) {
          Log.d("returnChromaKeyingPath", "ChromaKeying path is still null");
        } else {
          Log.d("returnChromaKeyingPath", "ChromaKeying path: " + path);
          PATH_GREEN_SCREEN = path + File.separator + "ht_gsseg_effect_config.json";
        }
      }
    });
    //手势配置文件
    // PATH_GESTURE = HTEffect.shareInstance().getGestureEffectPath() + File.separator + "ht_gesture_effect_config.json";
    BeautyManager.getPath("htGetGestureEffectPath", new BeautyManager.PathCallback() {
      @Override
      public void onPathReceived(String path) {
        if (path == null) {
          Log.d("returnGestureEffectPath", "GestureEffect path is still null");
        } else {
          Log.d("returnGestureEffectPath", "GestureEffect path: " + path);
          PATH_GESTURE = path + File.separator + "ht_gesture_effect_config.json";
        }
      }
    });
    //滤镜配置文件
    ...
    BeautyManager.getPath("htGetFilterPath", new BeautyManager.PathCallback() {
      @Override
      public void onPathReceived(String path) {
        if (path == null) {
          Log.d("returnFilterPath", "Filter path is still null");
        } else {
          Log.d("returnFilterPath", "Filter path: " + path);
          // 在这里处理接收到的路径
          PATH_BEAUTY_FILTER = path + File.separator + "ht_style_filter_config.json";
          PATH_EFFECT_FILTER = path + File.separator + "ht_effect_filter_config.json";
          PATH_FUNNY_FILTER = path + File.separator + "ht_haha_filter_config.json";
        }
      }
    });
    ...
    BeautyManager.getPath("htGetStylePath", new BeautyManager.PathCallback() {
      @Override
      public void onPathReceived(String path) {
        if (path == null) {
          Log.d("returnStylePath", "Style path is still null");
        } else {
          Log.d("returnStylePath", "Style path: " + path);
          // 在这里处理接收到的路径
          PATH_MAKEUPSTYLE = path + File.separator + "ht_makeup_style_config.json";
        }
      }
    });
